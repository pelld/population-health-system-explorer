...
(() => {
  ...
  if (!AE_MAP_NODES.some(node => node.id === "ucr-referrals")) {
    AE_MAP_NODES.push(AE_NODE("ucr-referrals","Two-hour UCR referrals received","professional",2,-2,840,{
      ...
      caution:"Recorded referrals do not measure all urgent community need or rejected and unrecorded demand.", evidence:"official", sources:["S18","S19"]
    }));
  }
  ...
  if (!AE_MAP_NODES.some(node => node.id === "ucr-care-contacts")) {
    AE_MAP_NODES.push(AE_NODE("ucr-care-contacts","Urgent community response care contacts","professional",2,12,940,{
      ...
      caution:"A referral can have more than one contact and contacts may fall in a different reporting month.", evidence:"official", sources:["S18","S19"]
    }));
  }
  ...
  if (!AE_MAP_NODES.some(node => node.id === "ucr-two-hour-achievement")) {
    AE_MAP_NODES.push(AE_NODE("ucr-two-hour-achievement","UCR response achieved within two hours","professional",2,26,840,{
      ...
      caution:"A higher percentage does not by itself show better clinical outcomes or more admissions avoided.", evidence:"official", sources:["S18","S19"]
    }));
  }
  ...
  AE_MAP_LINKS.push(
    AE_LINK("urgent-community-capacity","ucr-referrals","makes the urgent community route available","positive","official",["S18","S19"]),
    AE_LINK("ucr-referrals","ucr-care-contacts","leads to recorded assessment and intervention contacts","positive","official",["S18","S19"]),
    AE_LINK("ucr-care-contacts","ucr-two-hour-achievement","contributes to the response-time result","positive","official",["S18","S19"]),
    AE_LINK("care-home-urgent-events","ucr-referrals","can generate an urgent community referral","positive","published",["S18","S19"]),
    AE_LINK("ambulance-alternative","ucr-referrals","can refer into urgent community care","positive","hypothesis",["S18","S19"]),
    AE_LINK("ucr-care-contacts","other-professional-route","may prevent escalation to A&E when clinically appropriate","negative","hypothesis",["S18","S19"])
  );
})();
